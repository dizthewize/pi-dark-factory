 ...
 */
export function parseMeshTask(message: MeshMessage): FactoryTask | null {
  ...
  let priority: FactoryTask["priority"] = "medium";
 ...
 */
export function scanMeshInbox(
  messages: MeshMessage[],
  knownIds: Set<string>
): FactoryTask[] {
  const tasks: FactoryTask[] = [];
  for (const msg of messages) {
    if (knownIds.has(`MESH-${msg.id.slice(0, 8)}`)) continue;
