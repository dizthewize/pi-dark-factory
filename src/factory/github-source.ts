...
export interface GitHubSourceConfig {
  ...
  label: string;       // e.g., "pi-factory"
  token?: string;      // GITHUB_TOKEN env fallback
}
...
export async function fetchFactoryIssues(
  ...
  if (!token) throw new Error("GITHUB_TOKEN required for GitHub issue source");
...
function inferPriority(labels: string[]): FactoryTask["priority"] {
  const l = labels.map((x) => x.toLowerCase());
  if (l.includes("critical") || l.includes("p0")) return "critical";
  if (l.includes("high") || l.includes("p1")) return "high";
  if (l.includes("medium") || l.includes("p2")) return "medium";
  return "low";
}

export function issueToTask(issue: GitHubIssue): FactoryTask {
