...
import { FactoryState, FactoryTask, FactoryMode, LedgerEntry } from "../types.js";
...
export interface CycleDeps {
  ...
  factoryDir: string;
  cwd: string;
  // Optional extension bridges
  piWorkflows?: {
    plan?: (params: { input: string; output: string; name?: string }) => Promise<unknown>;
    execute?: (params: { name: string; tasks?: unknown[]; plan?: string; options?: unknown }) => Promise<unknown>;
  };
  piMesh?: {
...
export async function runOneCycle(deps: CycleDeps): Promise<{
  state: FactoryState;
  didWork: boolean;
}> {
...
async function runExecutor(
  task: FactoryTask,
  deps: CycleDeps
): Promise<{ ok: true; cost?: number } | { ok: false; error: string; cost?: number }> {
  ...
  try {
    ...
    })) as { cost?: { total?: number }; status?: string };
